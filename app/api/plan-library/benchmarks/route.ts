...
import { getVlmProvider, type VlmProviderName } from "../../../vlm/provider";
...
export async function POST(request: Request) {
  ...
  const providerNames = Array.isArray(body.providers)
    ? body.providers.filter((item): item is VlmProviderName => item === "openai" || item === "gemini")
    : ["openai", "gemini"] satisfies VlmProviderName[];
  if (!projectId || !pageId || !name || !providerNames.length) return Response.json({ error: "Project, page, name, and provider are required." }, { status: 400 });
  const project = await getOwnedPlanProject(user, projectId);
  if (!project) return Response.json({ error: "Plan project not found." }, { status: 404 });

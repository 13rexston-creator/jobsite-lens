...
// Real sheet titles often omit the discipline word entirely ("BUILDING A -
// LEVEL 1 - OVERALL PLAN", not "... OVERALL FLOOR PLAN") — the discipline
// word is a hint, not a requirement, once "level"/"overall"/"composite" and
// "plan" both appear.
 ...
 *
 * Real plan/count content is checked before the cover/index/admin demotion:
 * a genuine floor plan almost always also carries a routine "general notes"
 * or sheet-index callout box, and a naive keyword match on the whole page
 * would otherwise misfile the sheet as low-priority boilerplate. */
 ...
 *
 * Real plan/count content (0/1) is checked before the cover/index/admin
 * demotion (3): a genuine floor plan almost always also carries a routine
 * "general notes" or sheet-index callout box, and a naive keyword match on
 * the whole page would otherwise misfile the sheet as low-priority
 * boilerplate. The discipline word ("floor", "plumbing", ...) is a hint, not
 * a requirement — real sheet titles often just say "OVERALL PLAN" or
 * "LEVEL 1 PLAN" with no discipline word at all. */
export const fixturePagePrioritySql = sql<number>`case
  when length(trim(${pageText})) < 12 then 2
  when (instr(${compactPageText}, 'level') > 0 and instr(${compactPageText}, 'plan') > 0)
    or instr(${compactPageText}, 'overallplan') > 0 or instr(${compactPageText}, 'overallfloorplan') > 0
    or instr(${compactPageText}, 'overallplumbingplan') > 0 or instr(${compactPageText}, 'compositeplan') > 0
